...
public class MidiInput : MonoBehaviour
{
    ...
    {
        var v = InstrumentInput.inputNotes[noteNumber];
        if (v > 1.0f) {
            return v - 1.0f;
        } else if (v > 0.0) {
            return v;
    ...
    {
        return InstrumentInput.inputNotes[noteNumber] > 1.0f;
    }
    ...
    public static bool GetKeyUp (int noteNumber)
    {
        ...
        if (InstrumentInput.inputNotes[noteNumber] < 0.0f)
        {
            InstrumentInput.inputNotes[noteNumber] = 0.0f;
            return true;
        }
        else return false;
    }
...
//    float[] notes;
    ...
    {
        instance = this;
        InstrumentInput.inputNotes = new float[M.AllNotes];
    ...
    void Update ()
    {
        ...
        for (var i = 0; i < M.AllNotes; i++) {
            float x = InstrumentInput.inputNotes[i];
            if (x > 1.0f) {
                // Key down -> Hold.
                InstrumentInput.inputNotes[i] = x - 1.0f;
            } else if (x < 0) {
                // Key up -> Off.
                InstrumentInput.inputNotes[i] = 0.0f;
            }
        }
        ...
        while (true) {
            ...
            if (message.status == 0x90) {
                InstrumentInput.inputNotes[message.data1] = 1.0f / 127 * message.data2 + 1.0f;
            }

            // Note off message?
            if (message.status == 0x80 || (message.status == 0x90 && message.data2 == 0)) {
                InstrumentInput.inputNotes[message.data1] = -1.0f;
            }
